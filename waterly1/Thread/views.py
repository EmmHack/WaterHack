...
def login(request, in_or_out):
    ...
    next = request.GET.get(u'next', '/home/')
    valid = True
    ...
    if request.POST:
        ...
        if request.POST.get('delete') == 'delete':
            ...
            if User.objects.filter(username=username):
                ...
                edited = True
    form = register_form()
    return render(request, 'Thread/edit-profile.html',
                  {'form': form, 'user': User.objects.get(username=request.user.username), 'duplicate': duplicate,
                   'edited': edited, 'authenticated': authenticated})
        ...
        if not request.user.is_anonymous():
            ...
            try:
                ...
                if request.user.username == post.username:
                    post.text = new_text
                    post.save()
    return HttpResponseRedirect("/home/", D)
